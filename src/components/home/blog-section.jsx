...
const BlogSection = () => {
  ...
  const { data: blogs, isLoading, error } = useQuery({
    queryKey: ["childDevelopmentBlogs"],
    queryFn: fetchChildDevelopmentBlogs,
    staleTime: 5 * 60 * 1000,
  });
            ...
            <>
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 sm:gap-6 lg:gap-[65px] justify-items-center">
                {blogsList.map((blog) => (
                  <div
                    key={blog.id}
                    className="bg-[#F9F9F9] rounded-lg shadow-sm p-4 w-full max-w-[341px] h-[451px] flex flex-col"
                  >
                    {/* Image */}
                    <div className="w-full h-[230px] mb-4">
                      <img
                        ...
                        alt={blog.title}
                        className="rounded-[18px] w-full h-[230px] object-cover"
                        onError={(e) => {
                          e.currentTarget.src = "/placeholder.svg?height=230&width=316";
                        }}
                      />
                    </div>
                    {/* Content */}
                    <div className="flex flex-col justify-between flex-grow">
                      <div className="flex-grow">
                        <h3 className="text-[18px] font-bold text-[#1E3A8A] mb-2 line-clamp-2">
                          {blog.title}
                        </h3>
                        <p className="text-[18px] text-gray-700 leading-relaxed mb-4 line-clamp-3">
                          {blog.description}
                        </p>
                      </div>
                    </div>
